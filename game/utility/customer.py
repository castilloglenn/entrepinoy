...
class Customer(NPC):
    ...
    def __init__(
        ...
        while True:
            ...
            if (
                len(self.business_target["object"].queue)
                < self.business_target["object"].queue_limit
        ...
        self.queue_horizontal_space = (
            self.business_queue_horizontal_space * self.queue_number
        )
        ...
        self.target_points.append(
            (
                random.randint(self.rect.width, self.screen.get_width()),
            ...
            self.target_points.append(
                (
                    int(
                        ...
                        * (random.uniform(self.safe_spot[0], self.safe_spot[1]))
                    ),
                    int(
                        self.screen.get_height()
                        * (random.uniform(self.safe_spot[2], self.safe_spot[3]))
                    ),
                )
            )
        ...
        self.business_queuing_point_start = self.business_target["object"].rect.x + int(
            self.business_target["object"].rect.width
        ...
        self.target_slope = self.get_slope(
            self.current_position_in_float, self.target_points[self.target_index]
        )
    ...
    def animate_queue(self):
        try:
            ...
            if (
                self.is_standing
                and self.check_business_if_open()
                and self.check_business_if_queue_is_empty()
            ):
                if self.business_target["meta"]["queue_direction"] == "left":
                ...
                if self.queue_number > 0:
                    ...
                    is_in_line = (
                        person_ahead_of_line.rect.midbottom[1] - self.rect.midbottom[1]
                        <= self.destination_allowance
                    )

                    if not person_ahead_of_line.is_standing and not is_in_line:
                        queue_holder = current_queue[self.queue_number]
                        current_queue[self.queue_number] = current_queue[
                            self.queue_number - 1
                        ]
    ...
    def animate_movement(self):
        ...
        if self.speed_tick >= 1:
            ...
            try:
                ...
                allowable_gap = abs(target_point[1] - self.rect.midbottom[1]) + abs(
                    target_point[0] - self.rect.midbottom[0]
                )
            ...
            self.previous_position_in_float = copy.deepcopy(
                self.current_position_in_float
            )
            ...
            if int(self.previous_position_in_float[0]) != int(
                self.current_position_in_float[0]
            ) or int(self.previous_position_in_float[1]) != int(
                self.current_position_in_float[1]
            ):
                difference = (
                    round(self.current_position_in_float[0])
                    - round(self.previous_position_in_float[0]),
                    round(self.current_position_in_float[1])
                    - round(self.previous_position_in_float[1]),
                )
    ...
    def adjust_coordinates(self):
        if len(self.target_points) > len(self.exit_points) + 1:
            ...
            self.target_points = copy.deepcopy(
                self.target_points[: len(self.exit_points)]
            )
